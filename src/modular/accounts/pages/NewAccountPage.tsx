...
export const NewAccountPage = () => {
	...
	const saveAccount = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
	...
	return (
		<Layout>
			<form className="w-9/12 mt-sm space-y-sm">
				...
				{
					errors.length > 0 &&
					errors.map((error) => <p key={error} className="text-red-600 text-end mb-sm mt-md">{error}</p>)		
				}
