...
export const EmployeePage = () => {
	...
	const { employee_id, first_name, last_name1, last_name2, daily_salary, admision_date, status, created_date, updated_date, created_user_id } = state;
	...
	return (
		<Layout>
			<div>
				<form onSubmit={(event) => saveEmployeeData(event)} className="w-9/12 mt-sm">
					<h3 className="text-titleMd mb-xl uppercase">información del empleado con id: {employee_id}</h3>
					<EmployeeInformation created_date={created_date} created_user_id={created_user_id} updated_date={updated_date} />
					...
					<div className="flex flex-row -mx-sm mb-md">
						...
						<div className="flex flex-col items-start px-sm w-full mb-sm md:mb-0">
							<label className="block uppercase tracking-wide text-gray-900 text-lg font-bold mb-sm">PUESTO</label>
							<select value={job} onChange={({ target }) => setJob(target.value)} className="block w-full bg-gray-50 text-gray-800 border rounded-md py-sm px-md mb-xsm leading-tight focus:outline-none focus:bg-white">
					...
					<div className="flex flex-row -mx-sm mb-md">
						...
						/>
						<div className="flex flex-col items-start px-sm w-full mb-sm md:mb-0">
							<label className="block uppercase tracking-wide text-gray-900 text-lg font-bold mb-sm">ESTADO</label>
							<select value={newStatus} onChange={({ target }) => setNewStatus(target.value)} className="block w-full bg-gray-50 text-gray-800 border rounded-md py-sm px-md mb-xsm leading-tight focus:outline-none focus:bg-white">
